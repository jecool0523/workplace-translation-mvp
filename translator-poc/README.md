...
현재 PoC는 AI 모드가 켜져 있고 OpenAI API 키가 있으면 `expo-av`로 4초 음성을 녹음한 뒤 OpenAI STT로 텍스트화합니다. API 키가 없거나 녹음/STT가 실패하면 브라우저 Web Speech API를 시도하고, 지원하지 않는 환경에서는 샘플 발화로 대체합니다. 운영 서비스 전환 시에는 서버 STT 프록시, 현장 소음 모델, 안전 문구 검수, 네이티브 번역 검수가 필요합니다.
...
MVP 단계에서는 음성을 먼저 OpenAI STT로 텍스트화하고, 그 텍스트에 조사 데이터 컨텍스트를 붙여 번역한 뒤, 결과를 TTS로 송출합니다. 이 구조는 토큰 비용을 관리하기 쉽고 현장 용어 교정 루프를 만들기 쉽습니다.
