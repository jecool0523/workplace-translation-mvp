...
현재 웹 PoC는 브라우저가 지원하면 Web Speech API로 STT를 시도하고, 지원하지 않는 환경에서는 샘플 발화로 대체합니다. 운영 서비스 전환 시에는 모바일 네이티브 녹음, 서버 STT, 현장 소음 모델, 안전 문구 검수, 네이티브 번역 검수가 필요합니다.
...
MVP 단계에서는 음성을 먼저 텍스트로 만들고, 그 텍스트에 조사 데이터 컨텍스트를 붙여 번역한 뒤, 결과를 TTS로 송출합니다. 이 구조는 토큰 비용을 관리하기 쉽고 현장 용어 교정 루프를 만들기 쉽습니다.
